...
public class Nation
{
    ...
    {
        int monumentIncresePercentige = this.monuments.Sum(m => m.GetAffinity());
        double totalBendersPower = this.benders.Sum(b => b.GetPower());
        double totalPowerIncrese = totalBendersPower / 100 * monumentIncresePercentige;
    ...
    public override string ToString()
    {
        ...
        {
            result.AppendLine()
                .Append(string.Join(Environment.NewLine, this.monuments.Select(monumnet => $"###{monumnet}")));
        }
        else
        {
            result.Append(" None");
